...
// 🔥 ТОПОВЫЕ МОБИЛЬНЫЕ СТИЛИ (Отключаем скроллбары, синюю подсветку при тапе, настраиваем Safe Areas)
const styles = `
...
// 🔥 ПРОФЕССИОНАЛЬНАЯ МОБИЛЬНАЯ НАВИГАЦИЯ (Bottom Bar)
function MobileBottomNav({ activeTab, setActiveTab, onOpenPalette }) {
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 glass-panel border-t border-white/10 z-[40] px-8 py-2 flex justify-between items-center pb-[calc(env(safe-area-inset-bottom)+8px)]">
...
// 🔥 СТЕКЛЯННЫЙ HEADER (Для десктопа и тонкий для мобилок)
...
// 🔥 КАРТОЧКА: Адаптирована под тач, выглядит как премиальный актив
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="group relative w-full h-full cursor-pointer active:scale-[0.97] transition-transform" onClick={() => onOpenAnalytics(coll)}>
      <div className="bg-[#0f0f0f] rounded-2xl md:rounded-[24px] border border-white/5 overflow-hidden flex flex-col h-full shadow-lg relative z-10">
        ...
        {/* Изображение с эффектом глубины */}
        ...
        <div className="p-3 md:p-4 border-t border-white/5 bg-[#0a0a0a]">
          ...
          <div className="flex items-end justify-between">
            <div>
              <div className="text-sm md:text-lg font-black text-white leading-none">${formatMoney(displayPrice)}</div>
              {isPortfolio && quantity > 1 && <div className="text-[9px] text-slate-500 font-bold mt-1">${formatMoney(basePrice)} ea</div>}
            </div>
            <div className={`flex items-center gap-0.5 text-[10px] md:text-xs font-black bg-white/5 px-1.5 py-1 rounded-md ${isUp ? 'text-emerald-400' : 'text-rose-400'}`}>
...
function SearchPalette({ open, onClose, onAdd, onOpenAnalytics }) {
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[120] flex items-start justify-center pt-[env(safe-area-inset-top)] md:pt-20 px-4 bg-black/60 backdrop-blur-xl" onMouseDown={e => e.target === e.currentTarget && onClose()}>
      <motion.div initial={{ opacity: 0, scale: 0.95, y: -20 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="w-full max-w-[800px] glass-panel rounded-3xl overflow-hidden shadow-2xl flex flex-col mt-4 md:mt-0 h-[80vh] md:max-h-[70vh]" onMouseDown={e => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto flex-1 p-4 grid grid-cols-2 md:grid-cols-4 gap-4 pb-[calc(env(safe-area-inset-bottom)+20px)]">
          {loading ? <div className="col-span-full py-20 flex justify-center"><Loader2 className="animate-spin text-teal-500 w-8 h-8"/></div> : 
            results.map((item) => (
              <div key={item.id} onClick={() => { onOpenAnalytics(item); onClose(); }} className="bg-black/40 p-3 rounded-2xl border border-white/5 active:scale-95 cursor-pointer text-center relative group transition-all">
...
// 🔥 МОДАЛКА: Сделана как Bottom Sheet (Шторка) для мобилок
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-end md:items-center justify-center bg-black/80 backdrop-blur-sm" onMouseDown={e => e.target === e.currentTarget && onClose()}>
      ...
      <motion.div 
        ...
        className="w-full max-w-[1000px] bg-[#0a0a0a] rounded-t-[40px] md:rounded-[40px] border-t md:border border-white/10 p-6 md:p-10 shadow-2xl overflow-y-auto max-h-[90vh] md:max-h-[85vh] relative pb-[calc(env(safe-area-inset-bottom)+24px)]" 
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-2 flex flex-col items-center">
            {card.image_url && <img src={card.image_url} className="w-full max-w-[240px] md:max-w-[300px] drop-shadow-2xl mb-6" />}
            
            <div className="w-full bg-white/5 p-2 rounded-2xl flex gap-2">
              {!isPortfolio ? (
          ...
          {/* Правая колонка: График и статистика */}
...
function MainApp() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-teal-500/30">
      ...
      {/* Главный Контейнер: Отступ сверху для хедера, снизу для меню */}
      <div className="max-w-[1600px] mx-auto flex flex-col lg:flex-row pt-20 md:pt-28 px-4 md:px-8 gap-8 pb-32 md:pb-10">
        ...
        <main className="flex-1">
          ...
          {/* Статистика Портфеля (Компактная для мобилок) */}
          ...
          {(!loading || currentCards.length > 0) && activeTab === "explore" && (
            <div className="flex justify-between items-center mb-6 z-20 relative">
              ...
              <div className="relative">
                ...
                <AnimatePresence>
                  {sortOpen && (
                    <motion.div initial={{opacity:0, y:-10}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-10}} className="absolute top-full right-0 mt-2 w-48 bg-[#141414] border border-white/10 rounded-xl shadow-2xl py-2 overflow-hidden z-30">
          ...
          {loading && explorePage === 1 ? (
            ...
            <>
              {/* 🔥 ОБНОВЛЕННАЯ СЕТКА: На телефонах 2 колонки, карточки вытянутые */}
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-6">
