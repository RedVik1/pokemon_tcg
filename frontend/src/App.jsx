...
import {
  ...
  TrendingUp, TrendingDown, ExternalLink, ShoppingBag, X, Box, Download, Flame, AlertCircle, Compass
} from "lucide-react";
...
function MobileBottomNav({ activeTab, setActiveTab, onOpenPalette }) {
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[#111]/95 backdrop-blur-xl border-t border-white/10 z-[100] px-8 py-3 flex justify-between items-center pb-[calc(env(safe-area-inset-bottom)+12px)]">
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-[#111111] border-b border-white/5">
      <div className="max-w-[1600px] mx-auto px-4 md:px-8 h-16 flex items-center justify-between">
        ...
        {/* Mobile Header elements (just profile logout for mobile if needed, but we keep it clean) */}
        ...
        <div className="p-3 md:p-4 border-t border-white/5 bg-[#1a1a1a] relative z-30 transition-transform duration-500" style={{ transform: isHovered ? "translateZ(8px)" : "translateZ(0)" }}>
          ...
          <div className="flex items-center justify-between">
            <div className="flex flex-col">
              <div className="text-sm md:text-lg font-black text-white">${formatMoney(displayPrice)}</div>
              {isPortfolio && quantity > 1 && <div className="text-[8px] md:text-[9px] text-slate-500 font-bold">${formatMoney(basePrice)} / ea</div>}
            </div>
            <div className={`flex items-center gap-1 text-[9px] md:text-[10px] font-bold ${isUp ? 'text-emerald-400' : 'text-rose-400'}`}>
...
function AssetAnalyticsModal({ coll, onClose, onAdd, onDelete, isPortfolio }) {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md overflow-y-auto custom-scrollbar" onMouseDown={e => e.target === e.currentTarget && onClose()}>
      <button onClick={onClose} className="fixed top-4 right-4 md:top-8 md:right-8 z-[99999] bg-white/10 hover:bg-white/20 text-white p-3 md:p-4 rounded-full transition-all border border-white/20 shadow-2xl cursor-pointer">
        <X size={24} className="md:w-7 md:h-7" />
      </button>

      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative w-full max-w-[1200px] bg-[#111] border border-white/10 rounded-[30px] md:rounded-[40px] p-6 md:p-12 shadow-2xl my-10" onMouseDown={e => e.stopPropagation()}>
        <div className="flex justify-between items-start mb-6 md:mb-8 pr-12">
          <div>
            <div className="text-teal-500 font-black text-[10px] md:text-xs mb-1 md:mb-2 uppercase tracking-[0.2em]">{setName} • {rarity}</div>
            <h1 className="text-2xl md:text-5xl font-black text-white">{card.name}</h1>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="relative bg-[#1a1a1a] rounded-xl p-4 md:p-6 border border-white/5 flex flex-col items-center justify-center overflow-hidden">
            {card.image_url && <img src={card.image_url} className="w-full max-w-[200px] md:max-w-[300px] max-h-[35vh] md:max-h-none object-contain rounded-xl shadow-2xl mb-4 md:mb-6 relative z-10" />}
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between w-full md:w-auto gap-4">
                  <div className="flex bg-black rounded-lg p-1 border border-white/10 w-full sm:w-auto">
                    {['Raw', 'PSA 9', 'PSA 10'].map(g => (
                      <button key={g} onClick={() => setGrade(g)} className={`flex-1 sm:flex-none px-3 py-1 text-[10px] uppercase tracking-widest rounded-md font-bold transition-colors ${grade === g ? 'bg-teal-500 text-black' : 'text-slate-500 hover:text-white'}`}>{g}</button>
                    ))}
                  </div>

                  <div className="flex gap-1 bg-black rounded-lg p-1 w-full sm:w-auto">
                    {['1W', '1M', '1Y'].map(t => (
                      <button key={t} onClick={() => setTimeframe(t)} className={`flex-1 sm:flex-none px-3 py-1 text-xs rounded-md font-bold transition-colors ${timeframe === t ? 'bg-[#333] text-white' : 'text-slate-500 hover:text-white'}`}>{t}</button>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <main className="flex-1">
          ...
          {activeTab === "explore" && (
            <div className="bg-[#1a1a1a] rounded-xl p-4 md:p-6 mb-8 border border-white/5 shadow-lg">
              <h2 className="text-xl font-bold mb-4 hidden md:block">Explore Popular Cards</h2>
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <button onClick={() => setPaletteOpen(true)} className="bg-teal-500 text-black px-6 py-3 rounded-md font-bold hover:bg-teal-400 transition-colors hidden md:block">Search</button>
              </div>
            </div>
          )}
                    ...
                    {portfolioDistribution.slice(0, 3).map((item, i) => (
                      <div key={i} className="flex justify-between items-center bg-[#111] p-3 rounded-xl border border-white/5">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-bold text-white truncate max-w-[120px] md:max-w-none">{item.name}</span>
                        </div>
            ...
            <>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                ...
                <div className="relative">
                  <div onClick={() => setSortOpen(!sortOpen)} className="bg-[#1a1a1a] border border-white/5 text-sm text-slate-300 px-4 md:px-6 py-2 md:py-3 rounded-xl flex items-center gap-2 cursor-pointer hover:bg-white/5 transition-colors font-bold z-20 relative">
                    ⇅ Sort: {sortBy} ▾
                  </div>
                  {/* 🔥 ИСПРАВЛЕННЫЙ БАГ: motion.div initial */}
                  <AnimatePresence>
                    {sortOpen && (
                      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-full right-0 mt-2 w-56 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl z-30 py-2 overflow-hidden">
                        {(activeTab === "portfolio"
