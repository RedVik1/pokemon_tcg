...
function VaultCard({ coll, onOpenAnalytics, onAdd, onDelete, isPortfolio, quantity, inPortfolio }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="relative w-full h-full" style={{ perspective: "1500px" }}>
      <div onMouseEnter={() => setIsHovered(true)} onMouseLeave={onMouseLeave} onMouseMove={onMouseMove} onClick={() => onOpenAnalytics(coll)} style={{ transform: transformStyle, transition: isHovered ? "transform 0.1s ease-out" : "transform 0.6s cubic-bezier(0.23, 1, 0.32, 1)", transformStyle: "preserve-3d", willChange: "transform" }} className="group bg-[#1a1a1a] rounded-2xl border border-white/5 overflow-hidden cursor-pointer shadow-xl flex flex-col h-full relative z-10">
...
function MainApp() {
  ...
  return (
    <div className="min-h-screen bg-[#111] text-white font-sans selection:bg-teal-500/30">
        ...
        <main className="flex-1">
          ...
          {activeTab === "portfolio" && currentCards.length === 0 && !loading ? (
            <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in duration-500">
              <div className="w-24 h-24 bg-teal-500/10 text-teal-500 rounded-full flex items-center justify-center mb-6"><Box size={48} /></div>
            ...
            <>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                ...
                <div className="relative">
                  <div onClick={() => setSortOpen(!sortOpen)} className="bg-[#1a1a1a] border border-white/5 text-sm text-slate-300 px-6 py-3 rounded-xl flex items-center gap-2 cursor-pointer hover:bg-white/5 transition-colors font-bold z-20 relative">⇅ Sort: {sortBy} ▾</div>
                  {sortOpen && (
      ...
      {selectedColl && (<AssetAnalyticsModal coll={selectedColl} onClose={() => setSelectedColl(null)} onAdd={handleAdd} onDelete={confirmDelete} isPortfolio={activeTab === "portfolio"} />)}
    </div>
  );
}
