      ...
      <div className="flex flex-col h-full pt-safe">
        <div className="px-4 md:px-8 pt-4 md:pt-8">
          <div className="max-w-[800px] mx-auto w-full bg-[#141414] border border-white/[0.08] rounded-2xl overflow-hidden shadow-2xl">
            <form onSubmit={(e) => { e.preventDefault(); if (query.trim()) handleSubmit(query); }} className="p-4 flex items-center gap-3">
              ...
              />
              {query && (
                <button type="button" onClick={() => setQuery("")} className="p-1 text-slate-500 active:text-white transition-colors">
                  <X size={16} />
                </button>
              )}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-4 md:px-8 py-4">
          <div className="max-w-[800px] mx-auto w-full">
            {loading && debounced && (
              <div className="py-16 flex justify-center"><Loader2 className="animate-spin text-teal-500" /></div>
            )}
                ...
                {recentSearches.length > 0 && (
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button onClick={handleClearAll} className="text-slate-600 text-[10px] font-bold uppercase tracking-widest active:text-slate-400 transition-colors">
                        Clear All
                      </button>
                    </div>
            ...
            {showNoResults && (
              <div className="py-16 text-center">
                ...
                <p className="text-slate-500 text-sm font-semibold">No cards found for "{debounced}"</p>
              </div>
            )}
